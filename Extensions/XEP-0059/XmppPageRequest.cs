...
namespace Sharp.Xmpp.Extensions
{
    ...
    public class XmppPageRequest
    {
        ...
        {
            var setNode = Xml.Element("set", "http://jabber.org/protocol/rsm");
            var maxNode = Xml.Element("max").Text(FetchCount.ToString());

            setNode.Child(maxNode);

            if (After != null)
            {
                var afterNode = Xml.Element("after");
            ...
            if (Before != null)
            {
                var beforeNode = Xml.Element("before");
